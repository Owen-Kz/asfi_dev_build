...
if(VisitedUSername){
    fetch(`/getAwardsOf/${VisitedUSername.value}`, ()=>{
        method:"GET"
    }).then(res => res.json())
    .then(data =>{
        ...
        if(AwardArray.length > 0){
            AwardArray.forEach(Award => {
                ...
                awards_container.innerHTML+=`<div class="d-flex align-items-center mb-4">
                <span class="icon-md mb-0 bg-light rounded-3"><i class="fas">
                ...
                <div class="ms-3">
                    <h6 class="mb-0">${Award_title}</h6>
                    <p class="mb-0 small">${Awarded_by}</p>
                </div>
            </div>`
