...
const find_info = async (req,res) => {
   ...
   if(username_visitor){     
    ...
    db.query("SELECT * FROM followers WHERE followerUsername = ? AND followingUsername = ?", [visitor, username_visitor], async (ERR, followingScholar) => {
        ...
        db.query("SELECT * FROM user_info WHERE username = ? OR unique_code = ? OR email = ?", [username_visitor, username_visitor, username_visitor], async (err, scholar_user) => {
            if(err) throw err
           if(scholar_user[0]) {
          ...
          const Country = scholar_user[0]["country_of_residence"]
