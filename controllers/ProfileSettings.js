...
const ProfileSettings = async (req,res) =>{
    if(req.user){
        ...
        db.query("SELECT * FROM user_info WHERE username = ?", [username__], async (error, result) => {
            ...
            research_Gate:ReasearchGate_Link, Scopus:scopusLink_, youtube:YoutubeLink, GoogleScholar: GoogleScholarLink,WebOfScience_Link:WebOfScience_Link, OrchidLink:orchid_link, LinkedIn_Link:LinkedIn_Link, Academia:Academia})
                }
