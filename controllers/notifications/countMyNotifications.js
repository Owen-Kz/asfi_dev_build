...
const countMyNotifications = async (req,res) =>{
    try{
        ...
        const CheckNotification = await dbPromise.query(`SELECT COUNT(*) as notifications_count FROM new_notifications WHERE (recipient = ? OR recipient = ? OR isAnnouncement = 'yes') 
            AND (end_point != '' OR end_point NOT LIKE ?) AND status = 'unread'`, [username, userID, '%chat'])
