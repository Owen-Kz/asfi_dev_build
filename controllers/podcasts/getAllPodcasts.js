...
const getAllPodcasts = async (req,res) => {
   ...
   if(username_visitor){     
        ...
        return res.json({success:"podcasts", searchName: displayName, personTitle: title, personProfilePicture: profilePicture, accountStatus:accountStatus, visitor:visitor,searchUSERNAME:searchNameUser, accountType:account_type, PODCAST_ARRAY:PODCAST_ARRAY,      currentPage: pagePodcasts,
        totalPages: totalPagesPodcasts, podcast_owner_fullname: podcast_owner_fullname,  PodcastCount: podcastCount}) 
          
    
} 
else {
   return res.json({error:"erroro", status: "Invalid URL"})
}
