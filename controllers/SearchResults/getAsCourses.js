...
const getAsCourse = async (req, res) => {
        ...
        db.query("SELECT * FROM tutorials WHERE related_course_id != 'rtuT3g'", async (err, data) => {
            ...
            if (data.length > 0) {
                ...
                res.json({ message: "Success", AsCoursesArray: JSON.stringify(tutorialsWithOwners) });
            } else {
                res.json({ message: "No data Match your search",  AsCoursesArray:"[]"});
            }
        });
